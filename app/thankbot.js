...
export function addInvariants(ctx) {
  ...
  ctx.reason = ctx.stripped_text.substring(
    ctx.stripped_text.indexOf('for') + 3
...
async function check_invalid_dest({ dest, ctx }) {
  if (dest == undefined) {
    await slackClient.sendMessage(
      '[Error] Undefined user! did we add them to thanksbot?',
      ctx
    );
...
async function check_user_vote_today({ src, ctx }) {
  ...
  const msg = `<@${
    ctx.sender
  }>, you've already thanked someone in the last ${rate_limit_in_minutes} minutes! Try again in ${time_left_till_next_vote(
    src
  )} seconds!`;
